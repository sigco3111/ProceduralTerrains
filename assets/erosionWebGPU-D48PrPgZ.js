function ue(n){let s=n>>>0;return function(){s|=0,s=s+1831565813|0;let l=Math.imul(s^s>>>15,1|s);return l=l+Math.imul(l^l>>>7,61|l)^l,((l^l>>>14)>>>0)/4294967296}}function he(n,s,l){const h=[],d=[];let r=0;for(let t=-l;t<=l;t++)for(let y=-l;y<=l;y++){const o=y*y+t*t;if(o>=l*l)continue;const m=1-Math.sqrt(o)/l;h.push([y,t]),d.push(m),r+=m}for(let t=0;t<d.length;t++)d[t]/=r||1;return{offsets:h,weights:d,width:n,height:s}}function me({width:n,height:s,base:l,map:h,flow:d,erosionMask:r,depositionMask:t,params:y}){const o={...ee,...y},m=n*s;if(o.smoothing>0){const e=Float32Array.from(h);for(let g=1;g<s-1;g++)for(let p=1;p<n-1;p++){const u=g*n+p,C=(e[u]+e[u-1]+e[u+1]+e[u-n]+e[u+n])/5;h[u]=e[u]+(C-e[u])*o.smoothing}}const M=Math.max(0,Math.min(1,o.strength)),f=new Float32Array(m),v=new Float32Array(m),H=new Float32Array(m);let c=1e-6,x=1e-6,b=1e-6,P=1e-6;for(let e=0;e<m;e++)f[e]=l[e]+(h[e]-l[e])*M,v[e]=t[e],d[e]>c&&(c=d[e]),r[e]>x&&(x=r[e]),t[e]>b&&(b=t[e]);for(let e=0;e<s;e++)for(let g=0;g<n;g++){const p=e*n+g,u=g>0?f[p-1]:f[p],C=g<n-1?f[p+1]:f[p],W=e>0?f[p-n]:f[p],D=e<s-1?f[p+n]:f[p],a=Math.hypot(C-u,D-W);H[p]=a,a>P&&(P=a)}for(let e=0;e<m;e++)d[e]/=c,r[e]/=x,t[e]/=b,H[e]/=P;let E=1e-6;for(let e=0;e<m;e++)v[e]>E&&(E=v[e]);for(let e=0;e<m;e++)v[e]/=E;return{eroded:f,flow:d,erosionMask:r,depositionMask:t,sedimentMap:v,slopeMap:H}}const ee={seed:1,strength:1,droplets:6e4,maxLifetime:30,inertia:.05,sedimentCapacity:4,minSlope:.01,depositionRate:.3,erosionRate:.3,erosionRadius:3,evaporation:.02,gravity:4,initialSpeed:1,initialWater:1,thermalIterations:30,thermalStrength:.4,talus:.6,smoothing:.1},L=65536,ge=256,te=64;function ye(){return typeof navigator<"u"&&!!navigator.gpu}let R=null;function be(){return R||(R=(async()=>{const n=await navigator.gpu.requestAdapter();if(!n)throw new Error("no WebGPU adapter");const s=await n.requestDevice();return s.lost.then(()=>{R=null}),s})(),R.catch(()=>{R=null})),R}const ve=`
struct SimParams {
  width : u32,
  height : u32,
  brushCount : u32,
...
`;function S(n,s,l){return n.createBuffer({size:Math.max(4,s),usage:l})}async function O(n,s,l,h,d){const r=n.createCommandEncoder();r.copyBufferToBuffer(l,0,h,0,d),s.submit([r.finish()]),await h.mapAsync(GPUMapMode.READ);const t=new Int32Array(h.getMappedRange().slice(0));return h.unmap(),t}async function xe({width:n,height:s,heightmap:l,params:h,onProgress:d}){if(!ye())throw new Error("WebGPU not available");const r={...ee,...h},t=n*s,y=Math.max(0,Math.round(r.droplets)),o=await be();o.pushErrorScope("validation");const m=ue(r.seed|0||1),M=new Float32Array(Math.max(2,y*2));for(let a=0;a<y;a++)M[a*2]=m()*(n-1),M[a*2+1]=m()*(s-1);const f=he(n,s,Math.max(1,Math.round(r.erosionRadius))),v=new Float32Array(Math.max(4,f.offsets.length*4));for(let a=0;a<f.offsets.length;a++)v[a*4]=f.offsets[a][0],v[a*4+1]=f.offsets[a][1],v[a*4+2]=f.weights[a];const H=new Int32Array(t);for(let a=0;a<t;a++)H[a]=Math.round(l[a]*L);const c=GPUBufferUsage,x=S(o,80,c.UNIFORM|c.COPY_DST),b=S(o,t*4,c.STORAGE|c.COPY_SRC|c.COPY_DST),P=S(o,t*4,c.STORAGE|c.COPY_DST),E=S(o,t*4,c.STORAGE|c.COPY_SRC),e=S(o,t*4,c.STORAGE|c.COPY_SRC),g=S(o,t*4,c.STORAGE|c.COPY_SRC),p=S(o,t*4,c.STORAGE),u=S(o,v.byteLength,c.STORAGE|c.COPY_DST),C=S(o,M.byteLength,c.STORAGE|c.COPY_DST),W=S(o,t*4,c.MAP_READ|c.COPY_DST),D=[x,b,P,E,e,g,p,u,C,W];try{const a=o.queue;a.writeBuffer(b,0,H),a.writeBuffer(u,0,v),a.writeBuffer(C,0,M);const Y=new ArrayBuffer(80),ae=new Uint32Array(Y,0,8),ie=new Float32Array(Y,32,12);ae.set([n,s,f.offsets.length,0,0,Math.max(0,Math.round(r.maxLifetime)),0,0]),ie.set([r.inertia,r.sedimentCapacity,r.minSlope,r.depositionRate,r.erosionRate,r.evaporation,r.gravity,r.initialSpeed,r.initialWater,r.talus,r.thermalStrength,0]),a.writeBuffer(x,0,Y);const oe=o.createShaderModule({code:ve}),B=i=>o.createComputePipeline({layout:"auto",compute:{module:oe,entryPoint:i}}),q=B("droplets"),U=B("thermalMove"),z=B("thermalApply"),N=B("floorClamp"),G=(i,_)=>o.createBindGroup({layout:i.getBindGroupLayout(0),entries:_.map(([A,w])=>({binding:A,resource:{buffer:w}}))}),re=G(q,[[0,x],[1,P],[2,b],[3,E],[4,e],[5,g],[6,u],[7,C]]),ne=G(U,[[0,x],[8,b],[9,p]]),se=G(z,[[0,x],[10,b],[11,p]]),le=G(N,[[0,x],[1,P],[2,b]]),k=256,F=Math.ceil(y/k);for(let i=0;i<F;i++){const _=i*k,A=Math.min(k,y-_);a.writeBuffer(x,12,new Uint32Array([_,A]));const w=o.createCommandEncoder();w.copyBufferToBuffer(b,0,P,0,t*4);const T=w.beginComputePass();T.setPipeline(q),T.setBindGroup(0,re),T.dispatchWorkgroups(Math.ceil(A/te)),T.end();const I=w.beginComputePass();I.setPipeline(N),I.setBindGroup(0,le),I.dispatchWorkgroups(Math.ceil(t/64)),I.end(),a.submit([w.finish()]),((i&3)===3||i===F-1)&&(await a.onSubmittedWorkDone(),d==null||d((i+1)/F,"hydraulic"))}const X=Math.max(0,Math.round(r.thermalIterations));if(X>0&&r.thermalStrength>0){const i=o.createCommandEncoder();for(let _=0;_<X;_++){const A=i.beginComputePass();A.setPipeline(U),A.setBindGroup(0,ne),A.dispatchWorkgroups(Math.ceil(n/8),Math.ceil(s/8)),A.end();const w=i.beginComputePass();w.setPipeline(z),w.setBindGroup(0,se),w.dispatchWorkgroups(Math.ceil(t/64)),w.end()}a.submit([i.finish()]),await a.onSubmittedWorkDone(),d==null||d(1,"thermal")}const ce=await O(o,a,b,W,t*4),de=await O(o,a,E,W,t*4),fe=await O(o,a,e,W,t*4),pe=await O(o,a,g,W,t*4),$=await o.popErrorScope();if($)throw new Error(`WebGPU validation: ${$.message}`);const j=new Float32Array(t),J=new Float32Array(t),K=new Float32Array(t),Q=new Float32Array(t);for(let i=0;i<t;i++)j[i]=ce[i]/L,J[i]=de[i]/ge,K[i]=fe[i]/L,Q[i]=pe[i]/L;const V=me({width:n,height:s,base:l,map:j,flow:J,erosionMask:K,depositionMask:Q,params:r}),Z=new Float32Array(t);for(let i=0;i<t;i++)Z[i]=V.eroded[i]-l[i];return d==null||d(1,"done"),{delta:Z,...V}}catch(a){throw o.popErrorScope().catch(()=>{}),a}finally{for(const a of D)a.destroy()}}export{xe as erodeWebGPU,ye as isWebGPUErosionSupported};
