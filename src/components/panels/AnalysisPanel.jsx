...
export function AnalysisContent({ ctx }) {
  ...
  return <>
    ...
    <SelectRow label="최빈값" value={s.mode || 'elevation'} options={[
      { value: 'elevation', label: '고도' }, { value: 'slope', label: '경사' }, { value: 'normals', label: '법선' }, { value: 'curvature', label: '곡률' }, { value: 'waterDepth', label: '물 깊이' }, { value: 'biome', label: '바이옴 분포' }, { value: 'contribution', label: '페인트 + 스플라인 기여' },
    ]} onChange={(v) => ctx.onAnalysisMode(v)} />
    <SelectRow label="표시" value={s.display || 'overlay'} options={[{ value: 'overlay', label: '오버레이' }, { value: 'replace', label: '대체' }]} onChange={(v) => patch({ display: v, opacity: v === 'replace' ? 1 : s.opacity })} />
